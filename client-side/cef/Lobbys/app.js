...
function CreatePaintballLobby() {
    ...
    mp.trigger('CreatePaintballLobby', JSON.stringify({ "Name": name, "Password": password, "MaxPlayers": maxplayer, "MaxHealth": maxhp, "Map": maps[selectedMap], "Weapons": pickedWeapon }))
}
...
function SubmitEntry(index) {
    ...
    mp.trigger('JoinPaintballArena', index, password)
}
...
function SelectWeapon(weapon, el) {
    if (!selectedWeapons.includes(weapon.toLowerCase())) {
        ...
        pickedWeapon.push({"Weaponname": weapon});
    } else {
        ...
        for (let i = 0; i < pickedWeapon.length; i++) {
            if (pickedWeapon[i].Weaponname == weapon)
            {
                pickedWeapon.splice(i, 1);
                break;
            }
        }
    }
...
//SetPaintballData('{"Maps": ["Humane Labs", "Würfelpark"], "Weapons": ["Advancedrifle", "Bullpuprifle"]}')