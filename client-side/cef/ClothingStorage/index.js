var clothingStorageWindow = null;
let isOpen = false;

mp.events.add('Client:ClothesStorage:Open', (rawjson) => {
    try {
        if (clothingStorageWindow == null) {
            clothingStorageWindow = mp.browsers.new("package://cef/ClothingStorage/index.html");
            isOpen = true;
            mp.gui.cursor.show(true, true);
            mp.game.ui.displayRadar(false);
            mp.players.local.freezePosition(true);
            if (clothingStorageWindow != null) {
                clothingStorageWindow.execute(`SetMpDefinde()`);
                clothingStorageWindow.execute(`loadClothingStorage('${rawjson}')`);
...
mp.events.add('Client:ClothesStorage:Destroy', () => {
    try {
        if (clothingStorageWindow != null) {
            clothingStorageWindow.destroy();
            clothingStorageWindow = null;
            isOpen = false;
            ...
            mp.events.callRemote("Server:ClosedClothingSelection")
        }
    } catch (error) {
        mp.game.graphics.notify(error);
    }
...
mp.events.add("Client:ClothesStorage:TryClothes", (component, drawable, texture, is_accessories) => {
    ...
    mp.events.callRemote("Server:ClothingStorage:SetClothes", component, drawable, texture, is_accessories)
});
...
mp.events.add("Client:ClothingStorage:SelectCategory", (selection_id, is_accessories) => {
    ...
    if (selection_id == 6 && is_accessories) {
        ...
        clothingStorageWindow.execute(`PlayWatchAnimation(true)`)
        return;
    }
    clothingStorageWindow.execute(`PlayWatchAnimation(false)`)
})